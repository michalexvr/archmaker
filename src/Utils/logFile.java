...
public class logFile {
   ...
   public static void addReg(String mensaje)
    ...
    public static void ControlHora(String horaFecha)
    {
        String dir = System.getProperty("java.class.path").substring(System.getProperty("java.class.path").lastIndexOf(File.pathSeparator)+1);
        if(dir.lastIndexOf(File.separator)>=0){
            dir = dir.substring(0,dir.lastIndexOf(File.separator)+1);
        }
        archivo arch = new archivo(dir+"Control.txt");
        //SimpleDateFormat formato = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        //Date dat = new Date();
        //String fecha = formato.format(dat);
        arch.escribirLinea((new StringBuilder()).append(horaFecha).toString());

    }
    public static String getLastLineaControl()
    {
        String dir = System.getProperty("java.class.path").substring(System.getProperty("java.class.path").lastIndexOf(File.pathSeparator)+1);
        String LastFechaHControl=null;
        ...
        archivo arch = new archivo(dir+"Control.txt");
        LastFechaHControl= arch.retLastContenido(1);
            if(LastFechaHControl.equals(""))
            {
              LastFechaHControl=null;
            }
        
        return LastFechaHControl;
    }
}
